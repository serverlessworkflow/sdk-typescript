 ...
 * @param {Specification.DocumentTags} model The proxied object
 * @param {BuildOptions} options The build options to use
 * @returns {Specification.DocumentTags} The built object
 */
function buildingFn(model: Specification.DocumentTags, options: BuildOptions): Specification.DocumentTags {
  const instance = new Classes.DocumentTags(model);
  if (options.validate) instance.validate();
  return (options.normalize ? instance.normalize() : instance) as Specification.DocumentTags;
}
