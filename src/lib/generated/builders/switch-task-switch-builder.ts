 ...
 * @param {Specification.SwitchTaskSwitch} model The proxied object
 * @param {BuildOptions} options The build options to use
 * @returns {Specification.SwitchTaskSwitch} The built object
 */
function buildingFn(model: Specification.SwitchTaskSwitch, options: BuildOptions): Specification.SwitchTaskSwitch {
  const instance = new Classes.SwitchTaskSwitch(model);
  if (options.validate) instance.validate();
  return (options.normalize ? instance.normalize() : instance) as unknown as Specification.SwitchTaskSwitch;
}
