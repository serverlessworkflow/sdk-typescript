 ...
 * @param {Specification.RunTaskRunWorkflowInput} model The proxied object
 * @param {BuildOptions} options The build options to use
 * @returns {Specification.RunTaskRunWorkflowInput} The built object
 */
function buildingFn(
  model: Specification.RunTaskRunWorkflowInput,
  options: BuildOptions,
): Specification.RunTaskRunWorkflowInput {
  const instance = new Classes.RunTaskRunWorkflowInput(model);
  if (options.validate) instance.validate();
  return (options.normalize ? instance.normalize() : instance) as Specification.RunTaskRunWorkflowInput;
}
