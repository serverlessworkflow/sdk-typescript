 ...
 * @param {Specification.CallOpenAPIWithParameters} model The proxied object
 * @param {BuildOptions} options The build options to use
 * @returns {Specification.CallOpenAPIWithParameters} The built object
 */
function buildingFn(
  model: Specification.CallOpenAPIWithParameters,
  options: BuildOptions,
): Specification.CallOpenAPIWithParameters {
  const instance = new Classes.CallOpenAPIWithParameters(model);
  if (options.validate) instance.validate();
  return (options.normalize ? instance.normalize() : instance) as Specification.CallOpenAPIWithParameters;
}
