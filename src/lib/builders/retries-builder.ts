 ...
 */
function retriesBuildingFn(data: Specification.Retries): () => Specification.Retries {
  return () => {
    validate('Retries', data);
    return data;
  };
}
...
export function retriesBuilder(): Builder<Specification.Retries> {
  return builder<Specification.Retries>(retriesBuildingFn);
}
