...
import {
  ...
  ValidatorFactory,
  WorkflowValidator,
} from '../../src/model/workflow.validator';
...
describe('workflow builder', () => {
  ...
  it('should invoke validator class', () => {
    ...
    spyOn(workflowValidator, 'isValid').and.returnValue(false);
    spyOn(workflowValidator, 'validate').and.returnValue(new ValidationErrors([new ValidationError('any error')]));

    const validatorFactory = new ValidatorFactory();
    spyOn(validatorFactory, 'workflowValidator').and.returnValue(workflowValidator);

    try {
      new WorkflowBuilder(validatorFactory).build();
    } catch (error) {
