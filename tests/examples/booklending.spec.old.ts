...
describe('booklending workflow example', () => {
  it('should generate Workflow object', function () {
    const workflow = new WorkflowBuilder()
      ...
      .withVersion('1.0')
      .withStart(new StartBuilder().withName('Book Lending Request').build())
      .withStates([
        new EventStateBuilder()
          .withName('Book Lending Request')
          .withOnEvents([new OnEventBuilder().withEventsRef(['Book Lending Request Event']).build()])
      ...
      .withFunctions(new FunctionsBuilder().withURIDefinition('file://books/lending/functions.json').build())
      .withEvents(new EventsBuilder().withURIDefinition('file://books/lending/events.json').build())
      .build();

    const expected = JSON.parse(fs.readFileSync('./spec/examples/booklending.json').toLocaleString()) as any;
